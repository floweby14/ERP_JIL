...
<body>
    ...
    <div class="container-fluid page-header py-6 wow fadeIn" data-wow-delay="0.1s">
        <div class="container text-center pt-5 pb-3">
            <h1 class="display-4 text-white animated slideInDown mb-3">Data Master Guru</h1>
        </div>
    </div>

    <div class="card-body">
      <div class="basic-form">
          <form id="userForm" class="form-horizontal form-label-left" novalidate  action="<?= base_url('Data_Master/aksi_edit_guru')?>" method="post">
         <input type="hidden" name="id" value="<?= $data->id_user ?>">

              <div class="row">
                  <div class="mb-3 col-md-6">
                      <label class="form-label">NIK<span style="color: white;"> :</span></label>
                      <input type="Number" id="nik" name="nik" 
                      class="form-control text-capitalize" placeholder="NIK" oninput="maxLengthChecknip(this)" value="<?= $data->nik?>">
                      <script>
                  ...
                  <div class="mb-3 col-md-6">
                      <label class="form-label">Nama Guru<span style="color: white;"> :</span></label>
                      <input type="text" id="nama_guru" name="nama_guru" 
                      class="form-control text-capitalize" placeholder="Nama Guru" value="<?= $data->nama_guru?>">
                  </div>
                  <div class="mb-3 col-md-6">
                ...
                <?php elseif ($data->level == 3): ?>
                    <option value="1">Admin</option>
                    <option value="2">Kepala Sekolah</option>
                    <option value="3" selected>Kesiswaan</option>
                    <option value="4">Guru</option>
                <?php elseif ($data->level == 4): ?>
                    <option value="1">Admin</option>
                    <option value="2">Kepala Sekolah</option>
                    <option value="3">Kesiswaan</option>
                    <option value="4" selected>Guru</option>
                <?php endif; ?>
                  </select>
                </div>
            </div>
            <a href="<?= base_url('Data_Master/guru')?>" type="submit" class="btn btn-primary">Cancel</a></button>
